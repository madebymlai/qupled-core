...
class MasteryAggregator:
    ...
    def get_exercise_mastery(self, exercise_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'correct_rate': row['correct_reviews'] / row['total_reviews'] if row['total_reviews'] > 0 else 0.0,
    ...
    def update_exercise_mastery(
        ...
        """, (
            exercise_id, course_code,
            result['new_ef'], result['new_repetition'], result['new_interval'],
            result['next_review_date'].isoformat(),
            new_total, new_correct, mastery_level
        ))
        self.db.conn.commit()
    ...
    def get_core_loop_mastery(self, core_loop_id: int) -> MasteryStats:
        """
        ...
        rows = self.db.conn.execute("""
            ...
            JOIN exercise_core_loops ecl ON er.exercise_id = ecl.exercise_id
            WHERE ecl.core_loop_id = ?
        """, (core_loop_id,)).fetchall()

        if not rows:
            # No reviewed exercises - check how many exist
            count = self.db.conn.execute("""
                SELECT COUNT(*) FROM exercise_core_loops WHERE core_loop_id = ?
            """, (core_loop_id,)).fetchone()[0]
    ...
    def update_core_loop_mastery(self, core_loop_id: int, course_code: str) -> float:
        """
        ...
        self.db.conn.execute("""
            INSERT INTO student_progress
                (course_code, core_loop_id, mastery_score, total_attempts,
                 correct_attempts, review_interval, last_practiced, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
            ON CONFLICT(course_code, core_loop_id) DO UPDATE SET
                mastery_score = excluded.mastery_score,
                review_interval = excluded.review_interval,
                last_practiced = CURRENT_TIMESTAMP,
                updated_at = CURRENT_TIMESTAMP
        """, (
            course_code, core_loop_id, stats.average_mastery,
            total_attempts, correct_attempts, interval
        ))
        self.db.conn.commit()
    ...
    def get_topic_mastery(self, topic_id: int) -> MasteryStats:
        ...
        core_loops = self.db.conn.execute("""
            SELECT id FROM core_loops WHERE topic_id = ?
        """, (topic_id,)).fetchall()
    ...
    def update_topic_mastery(self, topic_id: int, course_code: str) -> float:
        ...
        """, (
            topic_id, course_code,
            stats.total_items, stats.mastered_count, mastery_pct
        ))
        self.db.conn.commit()
    ...
    def cascade_update(self, exercise_id: str) -> Dict[str, Any]:
        ...
        exercise = self.db.conn.execute("""
            SELECT e.course_code, ecl.core_loop_id, cl.topic_id
            FROM exercises e
        ...
        core_loop_id = exercise['core_loop_id']
        topic_id = exercise['topic_id']

        # Update core loop mastery
        cl_mastery = self.update_core_loop_mastery(core_loop_id, course_code)
        ...
        return {
            ...
            'topic': {
                'id': topic_id,
                'mastery_percentage': topic_mastery
            }
        }
    ...
    def get_exercises_by_mastery_level(
        ...
        if level == 'new':
            ...
            rows = self.db.conn.execute("""
                ...
                LIMIT ?
            """, (course_code, level, limit)).fetchall()
    ...
    def get_weak_core_loops(
        ...
        """
        rows = self.db.conn.execute("""
            SELECT sp.core_loop_id, sp.mastery_score, cl.name as core_loop_name,
